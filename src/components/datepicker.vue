    ...
    <div class="panel">
      ...
      <div class="panel-body">
        <div class="day-panel" v-show="showPanle == 'day'">
          ...
          <div class="week-view">
            <div class="week" v-for="item in weekList" v-bind:key="item">{{item}}</div>
          </div>
          <div class="day-view">
            <div class="day" :class="{isnow:isNow('day', item), disable:item.previousMonth || item.nextMonth || (day > item.value && year >= tempYear && month >= tempMonth), isselect: isSelect(item)}" @click="selectDay(item)" v-for="(item,index) in dayList" v-bind:key="index">{{item.value}}
            </div>
          </div>
        </div>
        <div class="month-panel" v-show="showPanle == 'month'">
          ...
          <div class="month-view">
            <div class="month" :class="{isnow:isNow('month',index), disable: month > index && year == tempYear}" @click="selectMonth(index)" v-for="(item,index) in monthList" v-bind:key="index">
              {{item}}
            </div>
          </div>
        </div>
        <div class="year-panel" v-show="showPanle == 'year'">
          ...
          <div class="year-view">
            <div class="year" :class="{isnow:isNow('year',item)}" @click="selectYear(item)" v-for="(item,index) in yearList" v-bind:key="index">{{item}}</div>
          </div>
        </div>
      </div>
    </div>
...
export default {
  ...
  computed: {
    dayList() {
      ...
      //当月1号是星期几 0~6 日~六
      let currentMonthStartDay = new Date(
        this.tempYear,
        this.tempMonth,
        1
      ).getDay();
  ...
  methods: {
    ...
    selectDay(sDay) {
      ...
      for (let i = 0; i < this.selectDate.length; i++) {
        if (
          this.tempYear == this.selectDate[i].year &&
          this.tempMonth == this.selectDate[i].month &&
          sDay.value == this.selectDate[i].day
        ) {
          flag = 1;
      ...
      if (sDate.currentMonth == true) {
        ...
        for (let i = 0; i < this.selectDate.length; i++) {
          if (
            this.tempYear == this.selectDate[i].year &&
            this.tempMonth == this.selectDate[i].month &&
            sDate.value == this.selectDate[i].day
          ) {
            flag = 1;
            break;
          }
